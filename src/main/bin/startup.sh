...
if [ "$daemon_mode" == "true" ]; then
  nohup $cmd -server ${JVM_OPTS} -jar conveyor-sets-${PRG_VERSION}.jar ${SPRING_ARGS} >/dev/null 2>&1 &
  echo $! > ${PRGDIR}/.pid
else
  exec $cmd -server ${JVM_OPTS} -jar conveyor-sets-${PRG_VERSION}.jar ${SPRING_ARGS}
fi
